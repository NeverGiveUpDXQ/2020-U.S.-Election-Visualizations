...
ggplot(county_full,
  ...
  labs(title = "2020 US Election Results: Absolute Difference",caption = "source: tonmcg & socviz")+
  theme_bw() +
  theme(plot.title = element_text(face = "bold"),
...
```{r, message=FALSE}
compare <- readr::read_csv(here::here("./data/clean/compare.csv"))

# plot
p <- ggplot(compare, aes(Year, Votes_PERCEN, 
                         frame = Year, 
                         size = Votes_PERCEN, 
                         group = Party,
                         colour = Party)) +
  ...
  labs(x = "year", y = "share", 
      title = "Presidential Elections:The Share of Electoral Votes for Dem. and Rep.", 
      subtitle = "Year : {frame_time} (Data for 2020 are provisional)",
       caption = "Source: socviz & Cook Political", fill = NULL)+
...
From this plot, we can see: Excluding year ftom 1875-1900, the votes percentage difference is relatively huge. At the same time, we can also see that no party has long been dominant. The share of the two parties is approximately cyclical.
...
exp_perc %>%
  ggparcoord(columns = c(2,3,5,9,11),
             groupColumn = 1,
             showPoints = T,
             scale = "uniminmax",
...
hc_title(text = "Total Disbursements By 2020 Joe Biden Committee By State") %>%
  ...
  hc_add_series_map(usgeojson, exp_flow_B,
   ...
   hc_colorAxis(min=1, max=max(exp_flow_T$value), minColor = "white", maxColor = "red",type="logarithmic", endOnTick=FALSE) %>%
   hc_legend(valueDecimals = 0, valueSuffix = "%") %>%
   hc_mapNavigation(enabled = TRUE)
```
