...
president2020 <- read_csv('./data/raw/Popular vote backend_1211.csv') %>%
  ...
  # make column `state` consistent & don't consider ("Maine 2nd District"    "Nebraska 2nd District" "Maine 1st District"  "Nebraska 1st District" "Nebraska 3rd District")
  select(stateid,state,dem_this_margin,dem_votes,rep_votes) %>%
  # we classfiy 3 levels: Dem. won, Rep. won and flipped states
  mutate(class = ifelse(stateid %in% c('AZ', 'WI', 'MI', 'PA', 'GA'),'States that flipped from 2016',ifelse(dem_votes>rep_votes,'Democratic win','Republican win')))
...
```{r}
attach(president2020)
president2020 <- readr::read_csv(here::here("./data/clean/president2020.csv"))

president2020$dem_this_margin[stateid %in% c('AZ', 'WI', 'MI', 'PA', 'GA')] = NA

president2020<- president2020%>%
  select(state,dem_this_margin)

colors = c("darkred","white","darkblue")

highchart() %>%
  hc_title(text = "2020 Election Result Margin By State (w/o filpped state)") %>%
  ...
  hc_add_series_map(usgeojson, president2020,
   ...
   hc_legend(title = list(text = "Dem'20 Margin"),
              enabled = TRUE)
```
...
highchart() %>%
  hc_title(text = "2020 Election Result Margin By Filpped State") %>%
  hc_subtitle(text = "Double click the state area to zoom in") %>%
  hc_add_series_map(usgeojson, president2020,
    name = "Margin",
    value = "dem_this_margin", joinBy = c("name", "state"),
   ...
   hc_colorAxis(min = min(president2020$dem_this_margin), max = max(president2020$dem_this_margin),
...
Also, we compared election results between 2016 and 2020 on the state level, and we found:
...
- Joe Biden enlarged the margin of **Minnesoda**(1.52%D -> 7.11%D) and **New Hampshire**(0.37%D -> 7.35%D) that Hillary Clinton won in 2016.
...
ggplot(elections_historic_new, aes(x = popular_pct, 
  ...
  labs(x = "Winner's share of popular vote", 
       ...
       title = "Presidential Elections: Popular & Electoral College Margins", subtitle = "From 1824 to 2020",
...
ts1 <- ts %>%
  ...
  hc_yAxis(title = list(text = "Contribution Amount (US Dollars)"),
           ...
           max = 14000000) %>%
  hc_title(text = "<b>Individual Contributions to US Presidential Candidates 2020</b>") %>%
