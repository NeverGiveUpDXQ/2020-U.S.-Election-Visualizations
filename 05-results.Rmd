...
```{r,echo=FALSE}
d1 <- statepop1 %>% select(abbr, BIDEN_RECEIPT_AMOUNT,TRUMP_RECEIPT_AMOUNT) %>%
...
d1 %>% arrange(desc(Contribution)) %>%
  ...
  hc_subtitle(text = "Click on the legend to show selected candidates; click on a state and drag horizontally to zoom in.") %>%
  hc_exporting(enabled = TRUE)
```

The bar chart above displays how much Joe Biden and Donald Trump have raised in each state in 2020, and you can zoom in to see each state's contribution amounts in detail. Joe Biden received solid support from **California** and **New York** which contributed 127M dollars and 63M dollars respectively, more than twice as contributed to Donald Trump. Besides receiving 58M dollars from **California**, Donald Trump also raised significant amounts from **Texas** and **Florida** equal to 52M and 48M dollars respectively, much more than Biden. We found that these are consistent with the final voting results as Biden has won California and New York with a margin larger than 20% whereas Trump won Florida and Texas, two of the battleground states.
...
knitr::kable(disbursement_summary, caption = "Disbursement Summary by Candidate",
             ...
             digits = 2,align = "l",full_width = T)
```
In the boxplot above, we used the log10 scaled x-axis to reduce skewness of the disbursement distribution. There are outliers on both ends, but more outliers clustering at the higher expenditure level rather than lower expenditure level, making the distributions a little right skewed. The median spending amounts are almost equal for two candidates.


### Top 5 Presidential Campaigns Spent the Most
```{r, message=FALSE, warning=FALSE}
...
ggplot(com_exp, aes(x = total, y = reorder(committee_name, total), color=PoliticalParty, label=total)) +
  ...
  labs(x = "Total Expenditure (Million $)", y = "Candidate", caption = "Source: fec.gov", 
       title ="Top 5 Presidential Campaigns Made the Most Disbursements", 
...
Among 5 committees that spent the most during this year, 4 of them are Democratics, 1 of them is Republican. This might relate to a fact the Democratic party has received more contributions than the Republican party. One interesting finding is that Mike Bloomberg's Committee spend the most among all of the committees. This is not expected since Bloomberg quit the election race very early in March 2020. According to the article from [The Guardian](https://www.theguardian.com/us-news/2019/nov/23/michael-bloomberg-refuse-donations-presidential-bid), all of those expenditures came from Bloomberg's personal fortune instead of campaign contributions. Therefore, Mike Bloomberg's campaign is an outlier. Joe Biden's committee and Donald Trump's committee were as expected the next most-spending campaigns, but Biden has spent 300 Million dollars more than Trump.
