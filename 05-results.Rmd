...
president2020 <- read_csv('./data/raw/Popular vote backend_1211.csv') %>%
...
We compared how vote shares have shifted in elections from 1976 to 2016
...
highchart(type = "map") %>% 
  hc_add_series(data = history_election_ds,
                name = "Popular Vote Shares for Rep.",
                mapData = usgeojson,
                joinBy = c("name","state"),
                borderWidth = 0.05,
  ...
  hc_legend(layout = "vertical", 
            align = "right",
            title = list(text = 'Share')) %>% 
  hc_add_theme(hc_theme_smpl()) %>%
  hc_motion(
    enabled = TRUE,
    axisLabel = "year",
    labels = as.character(sort(unique(history_election$year),decreasing = FALSE)),
    magnet = list(
      round = "floor" 
    )
  )
...
ggplot(elections_historic_new, aes(x = popular_pct, 
  ...
  annotate(geom = "rect", xmin = 0.37, xmax = 0.498, ymin = 0.502, ymax = 0.85, fill = "red", alpha = 0.2) + 
  annotate(geom = "text", x = 0.37, y = 0.9, label = "High electoral vote share\nbut low popular vote share", hjust = 0)+
  labs(x = "Winner's share of popular votes", 
       y = "Winner's share of electoral college votes", 
       title = "Presidential Elections: Popular vs. Electoral College Margins", subtitle = "From 1900 to 2020",
...
- Ideally, we hope the president-elect also had high share of popular vote, however, the data points falling in the red rectangular were not this case. This can be explained by [how the US presidential election works](https://en.wikipedia.org/wiki/United_States_presidential_election).
...
```{r, fig.height=15,fig.width=12,message=FALSE, echo=FALSE}
# https://rstudio-pubs-static.s3.amazonaws.com/10130_e7ab01469cdf4ddb85e3510b0844885e.html
# https://rdrr.io/cran/micromapST/man/panelDesc.html
library(micromapST)
popular_vote <- readr::read_csv(here::here("./data/raw/Popular vote backend_1211.csv"))
statepop1 <- readr::read_csv(here::here("./data/d3csv/statepop1.csv"))
statepop2 <- statepop1 %>%
  left_join(popular_vote[,c(2:14,20,21)],by = c("abbr" = "stateid" ))
row.names(statepop2) <- statepop2$abbr

# column formats
statepop2$dem_this_margin <- round(statepop2$dem_this_margin,2)
statepop2$`2016 Margin`<- round(statepop2$`2016 Margin`,2)
statepop2$BIDEN_C_PERC <- round(statepop2$BIDEN_RECEIPT_AMOUNT/statepop2$ALL_RECEIPT_AMOUNT,2)
statepop2$TRUMP_C_PERC <- round(statepop2$TRUMP_RECEIPT_AMOUNT/statepop2$ALL_RECEIPT_AMOUNT,2)


#popular_vote_battlegrounds <- popular_vote[5:19,]
#popular_vote_flipped <- popular_vote[popular_vote$stateid %in% c('AZ', 'WI', 'MI', 'PA', 'GA'),]


#df<- as.data.frame(statepop2[statepop2$abbr %in% popular_vote_battlegrounds$stateid,])
#rownames(df) <- df[,1]
#df$zero <- round(df$ALL_RECEIPT_AMOUNT/10000000000000,0)

df1<- as.data.frame(statepop2)
rownames(df1) <- df1[,1]
df1$zero <- round(df1$ALL_RECEIPT_AMOUNT/10000000000000,0)
df1$BIDEN_C <- round(df1$BIDEN_RECEIPT_AMOUNT/1000000,0)
df1$TRUMP_C <- round(df1$TRUMP_RECEIPT_AMOUNT/1000000,0)

panelDesc = data.frame(type = c("mapmedian", "id", "dot","arrow", "ctrbar", "normbar"), 
            lab1 = c("","","Dem's Margin","Margin Shift","Contributions To","Contributions Per Voter"), 
            lab2 = c("", "", "2020","2016 to 2020","Biden vs. Trump (2020)","Biden vs. Trump (2020)"),
            lab3 = c("", "", "Democrats' Margin", "Democrats' Margin", "Million Dollars", "Percent"),
            col1 = c(NA, NA, 20, 28, 29, 10), 
            col2 = c(NA, NA, NA, 21, 30, 11))
            #colSize = c(NA,NA,22,22,28,28))

#micromapST(df, panelDesc, sortVar = 20, ascend = FALSE)#, title = c("Title","sub"))



micromapST(df1, panelDesc, sortVar = 20, ascend = FALSE, title = c("2020 US Presidential Election Results by State","Margins, Margin shifts, and Contributions"))
...
The bar chart above focuses on the total amounts raised from each state by the final presidential candidates Joe Biden and Donald Trump, and it displays top 5 states that contributed the most to each candidate. Joe Biden received solid support from California and New York which contributed 127M dollars and 63M dollars respectively, more than twice as contributed to Donald Trump. Donald Trump raised significant amounts from Texas and Florida equal to 52M and 48M dollars respectively, much more than Biden. This is consistent with the voting results as Biden has won in both states with a margin larger than 20%. We found that these are consistent with the final voting results as Biden has won California and New York with a margin larger than 20% whereas Trump won Florida and Texas, two of the battleground states.
...
If we instead take into account the registered voters in each state and calculate contributions per registered voter, we found that District of Columbia has the highest average contribution to Biden, which is 37 dollars per registered voter, way more than any other state. Remarkably, in the election Biden won 93% of total votes in DC. In addition, Wyoming surprisingly has contributed the most to Trump on an average level. Wyoming has the lowest percentage of registered voters among all states that only 47.4% of its population registered to vote, and its support for Trump was reflected on the voting results as Trump won the state with a margin over 40%.
...
To obtain a better understanding about individual contributors, we extracted keywords from contributors occupations and separated them by candidate, so that we were able to draw word a cloud for each candidate.
...
Comparing the difference between two candidates, more than half of 10 top occupations of Joe Biden's contributors are legal or technical professional positions, whereas more than half of 10 top occupations of Donald Trump's contributors are jobs in business. 
...
colors <- c("white", "lemonchiffon", "orange")
highchart(type="map") %>%
hc_title(text = "Total Disbursements By All Presidential Candidates 2020 By State") %>%
  ...
  hc_add_series_map(usgeojson, state_total,
   ...
   hc_colorAxis(min=1e6, type="logarithmic", endOnTick=FALSE, stops = color_stops(n=length(colors), colors = colors)) %>%
   hc_legend(title = list(text = "US Dollar Amount"), valueDecimals = 0, valueSuffix = "%") %>%
   hc_mapNavigation(enabled = TRUE) %>% 
  hc_exporting(enabled = TRUE)
```
...
hc_title(text = "Total Disbursements By 2020 Joe Biden Committee By State") %>%
  ...
  hc_add_series_map(usgeojson, exp_flow_B,
   ...
   hc_colorAxis(min=1e3, max=max(exp_flow_T$value),type="logarithmic", stops = color_stops(n=length(colors), colors = colors), endOnTick=FALSE) %>%
   hc_legend(title = list(text = "US Dollar Amount"), valueDecimals = 0, valueSuffix = "%") %>%
   hc_mapNavigation(enabled = TRUE) %>% 
  hc_exporting(enabled = TRUE)
```
