...
First, our purpose is to compare share of popular votes for **republican** in state level from 1976 to 2016. We cleaned this dataset following steps including:
...
We then filter all outliers and want to revise them. Note that it involves 2 state: **District of Columbia** and **Minnesota**. After checking data, we know that Republican hasn't ever won D.C. and popular votes have always relatively low. So, it is reasonable. However, the share of *Minnesota* for Republican is abnormal. It is impossible that all voters chose one party.
...
knitr::kable(data.frame(
                description = c('year','state','type'),
                category = c("Which election year?","State name","If Share of popular votes for Rep. < 0.25, type = 0, else if share < 0.5, type = 1, else if share < 0.75, type = 2, else type = 3")
              ), 
