...
First, our purpose is to compare popular votes for democrat and republican in state level from 1976 to 2016. We cleaned this dataset following steps including:
...
- Add a new column `ratio` = candidatevotes_democrat/(candidatevotes_democrat+candidatevotes_republican.)
- Then convert `ratio` continuous variable to categorical variable `ratio_new`. 
...
# only choose democrat lines
index <- temp$party == 'democrat'

history_data <- temp%>%
  group_by(year,state)%>%
  summarise(ratio = candidatevotes/sum(candidatevotes))%>%
  ungroup()

history_data <- history_data[index,]
...
write.csv(history_data, file = here::here("data", "clean", "1976-2016election.csv"),row.names=FALSE)
```

After cleaning, it contains 4 columns and  records.
```{r, echo=FALSE, results='asis'}
knitr::kable(data.frame(
                description = c('year','state','party','candidatevotes'),
                category = c("Which election year?","State name",
                "Party name: Dem or Rep?", "Popular votes")
              ), 
