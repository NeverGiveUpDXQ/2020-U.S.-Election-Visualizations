...
Most of our small auxiliary datasets have been pre-processed by data providing authorities. Hence, we will only focus on preprocessing the major four datastes in this and the following chapter. 

## Election Result Data Transformation 
In this step, we need to cleanup two election result data files: 1976-2016 historical election result and 2020 election popular vote share and vote margin. We began with cleaning the historical election result data file. 
...
- Add a new column `class` to indicate if it is `States that flipped from 2016` or `Democrats won`, or `Republicans won`. We define a state as `States that flipped from 2016` if it is one of **Arizona**, **Wisconsin**, **Michigan**, **Pennsylvania**, **Georgia**. Otherwise, we compared their vote for Democrats and vote for Republicans to determine their class.
